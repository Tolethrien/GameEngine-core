...
export default class LightMap extends System {
  ...
  onUpdate(): void {
    ...
    ctx.drawImage(
      this.lightMapCanvas,
      this.camera.position.get.x,
      this.camera.position.get.y
    );
  }
  worldToScreen(id: string) {
    return {
      x:
        (this.positions.get(id)!.vec.get.x - this.camera.position.get.x) /
        this.camera.zoom,
      y:
        (this.positions.get(id)!.vec.get.y - this.camera.position.get.y) /
        this.camera.zoom,
    };
  }
}
