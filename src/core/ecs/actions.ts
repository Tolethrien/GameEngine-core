...
export abstract class ActionsController {
  ...
  getAction(name: string) {
    if (!this.actions.has(name)) throw new Error("trying to get or use action that doesn't exist");
    return this.actions.get(name)!;
  }
}
export const useAction = (name: string, props?: UseProps) => {
  if (!Engine.actions)
    throw new Error("You try to use Actions, but there are no action controller asigned to Engine");
